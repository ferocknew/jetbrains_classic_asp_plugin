...
/**
 * 使用IntelliJ内置HTML支持的混合语法高亮器
 * 在ASP部分使用自定义解析器，在HTML部分使用内置HTML解析器
 */
public class IntelliJHTMLHybridHighlighter extends SyntaxHighlighterBase {
    ...
    public Lexer getHighlightingLexer() {
        return new IntelliJHTMLHybridLexer();
    }
    ...
    /**
     * 使用IntelliJ内置HTML支持的混合词法分析器
     */
    private static class IntelliJHTMLHybridLexer extends LexerBase {
        ...
        private boolean htmlLexerInitialized = false;

        public IntelliJHTMLHybridLexer() {
            // 延迟初始化HTML词法分析器
        }

        private void initHTMLLexer() {
            if (htmlLexerInitialized) {
                return;
            }
            ...
            try {
                ...
                htmlLexerInitialized = true;
            } catch (Exception e) {
                // 如果无法获取HTML词法分析器，使用简化处理
                this.htmlLexer = null;
                htmlLexerInitialized = true;
            }
        }
        ...
        private void processHTMLToken() {
            if (htmlLexer != null) {
                // 使用内置HTML词法分析器
                processWithHTMLLexer();
            } else {
                // 使用简化HTML处理
                processSimplifiedHTML();
            }
        }

        private void processWithHTMLLexer() {
            // 这里可以集成内置HTML词法分析器
            // 由于复杂性，暂时使用简化处理
            processSimplifiedHTML();
        }

        private void processSimplifiedHTML() {
            char currentChar = buffer.charAt(currentPosition);
